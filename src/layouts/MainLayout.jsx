...
export default function MainLayout({ children }) {
    ...
    return (
        <div className="min-h-screen bg-[#090A0F] font-sans text-zinc-100 selection:bg-indigo-500/30 flex flex-col relative overflow-hidden">
            {/* Latar Belakang Glow Global */}
            <div className="absolute top-[-20%] left-1/2 -translate-x-1/2 w-[1000px] h-[800px] bg-indigo-600/10 blur-[150px] rounded-full pointer-events-none z-0"></div>
            ...
            <header className="fixed top-0 z-50 w-full border-b border-[#1E1F2E] bg-[#090A0F]/80 backdrop-blur-md">
                <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    {/* Menu Desktop */}
                    <nav className="hidden md:flex items-center gap-2 bg-[#13141C] p-1.5 rounded-2xl border border-[#262833]">
                        {navLinks.map((link) => {
                            ...
                            return (
                                <Link
                                    ...
                                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
                                        isActive
                                            ? 'bg-[#2E2459] text-white shadow-md'
                                            : 'text-zinc-400 hover:text-zinc-200 hover:bg-[#1A1C26]'
                                    }`}
                                >
                    ...
                    <div className="hidden md:flex items-center gap-4">
                        {!isAuthenticated ? (
                            ...
                            <button
                                className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-indigo-500/25"
            ...
            <footer className="border-t border-[#1E1F2E] bg-[#0E0F15] py-12 relative z-10">
                <div className="max-w-6xl mx-auto px-6 text-center text-zinc-500">
                    ...
                    <p className="text-sm mb-2 font-medium">
                        CeraScan AI - Solusi Visi Komputer Industri.
                    </p>
                    <p className="text-xs text-zinc-600">
                        Dibangun berdasarkan arsitektur Deep Learning VGG-19.
                    </p>
                </div>
            </footer>
        </div>
