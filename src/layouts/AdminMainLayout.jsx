...
export default function AdminMainLayout({ children, activeMenu, setActiveMenu }) {
    ...
    return (
        <div className="flex h-screen w-full bg-[#090A0F] font-sans text-zinc-100 selection:bg-indigo-500/30 overflow-hidden">
            {/* 1. SIDEBAR KIRI */}
            ...
            <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#090A0F] relative">
                ...
                <div className="absolute top-0 right-0 w-[600px] h-[400px] bg-indigo-600/5 blur-[120px] rounded-full pointer-events-none"></div>

                {/* Top Navbar */}
                <header className="h-16 flex items-center justify-between px-8 border-b border-[#1E1F2E] bg-[#0E0F15]/80 backdrop-blur-md shrink-0 z-10">
                    {/* Breadcrumb / Judul */}
                    <div className="flex items-center gap-2 text-sm font-medium">
                        <span className="text-zinc-500">Dashboard</span>
                        <ChevronRight className="w-4 h-4 text-zinc-700" />
                        <span className="text-zinc-200 capitalize">
                            {currentMenu.replace('_', ' ')}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <button className="relative text-zinc-400 hover:text-white transition-colors">
                            ...
                            <span className="absolute -top-1 -right-1 w-2 h-2 bg-indigo-500 rounded-full border border-[#0E0F15]"></span>
                        </button>
                    </div>
                </header>
