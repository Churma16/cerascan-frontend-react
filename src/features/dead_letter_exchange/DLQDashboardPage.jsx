...
export default function DLQDashboardPage() {
    ...
    const handlePurge = async () => {
        if (!window.confirm('Apakah Anda yakin ingin menghapus semua pesan di DLQ? Tindakan ini tidak dapat dibatalkan.')) {
            return;
        }
        setIsPurging(true);
        try {
    ...
    return (
        <PageWrapper>
            <div className="relative flex-1 flex flex-col overflow-y-auto px-6 py-4">
                {/* Toast Notification */}
                {toast && (
                    <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg border transition-all duration-300 transform translate-y-0
                        ${toast.type === 'error' 
                            ? 'bg-red-50 text-red-700 border-red-200' 
                            : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center 
                            ${toast.type === 'error' ? 'bg-red-100' : 'bg-emerald-100'}`}>
                            {toast.type === 'error' ? <AlertTriangle className="w-3 h-3" /> : <Check className="w-3 h-3" />}
                        </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    <AdminPageHeader 
                        title="Antrean Gagal (DLQ)" 
                        desc="Kelola event dan tugas yang gagal diproses oleh worker antrean (RabbitMQ)." 
                    />
                ...
                <div className="bg-white border border-gray-100 p-6 rounded-xl shadow-[0_4px_25px_rgb(0,0,0,0.02)] mb-6 flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs font-bold text-gray-400 tracking-wide uppercase mb-0.5">Total Kegagalan Antrean</p>
                        <h3 className="text-2xl font-black text-[#042B1F]">
                            {messages.length} <span className="text-sm font-normal text-gray-500">pesan tertahan di DLQ</span>
                        </h3>
                    </div>
                </div>
                ...
                <div className="bg-white border border-gray-100 rounded-xl shadow-[0_4px_25px_rgb(0,0,0,0.02)] overflow-hidden flex-1 flex flex-col">
                    {isLoading ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-20 text-gray-400">
                            ...
                            <p className="text-xs font-bold">Memuat data dari antrean gagal...</p>
                        </div>
                    ) : messages.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center py-20 text-gray-400">
                            ...
                            <p className="text-xs text-gray-400">Tidak ada pesan yang tertahan di antrean gagal (DLQ).</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-gray-100 bg-gray-50/50">
                                        <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Event / Routing Key</th>
                                        <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Payload ID</th>
                                        <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Waktu Masuk</th>
                                        <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider text-right">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {messages.map((msg, index) => (
                                        <tr key={msg.messageId || index} className="hover:bg-gray-50/40 transition-colors">
                        ...
                        <div className="bg-white border border-gray-100 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[80vh]">
                            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-sm font-black text-[#042B1F]">Detail Payload Pesan</h3>
                                </div>
                                <button 
                            ...
                            <div className="p-6 overflow-y-auto bg-gray-50 font-mono text-xs text-gray-700 flex-1">
                                <div className="mb-4">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Routing Key:</p>
                                    <p className="font-bold text-gray-800">{selectedMessage.routingKey}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-1">Content Body (JSON):</p>
